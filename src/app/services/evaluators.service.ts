...
export class EvaluatorsService {
  ...
  private ensUrl = 'api/ens';  // URL to in-memory-data-service api
  ...
  getEvaluators (): Observable<Evaluator[]> {
    ...
    /** GET ENS from the server */
    getENS (): Observable<ENS[]> {
        return this.http.get<ENS[]>(this.ensUrl)
          .pipe(
            tap(_ => this.log('fetched ENS')),
            catchError(this.handleError<ENS[]>('getENS', []))
          );
      }
